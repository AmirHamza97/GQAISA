...
%Exponential Distribution Optimization (EDO) Algorithm
%The paper title is "Exponential Distribution Optimizer (EDO): A Novel Math-Inspired Algorithm for Global Optimization and Engineering Problems"
%Journal of Artificial Intelligence Review, 2023
...
%BestFitness--the optimal Fitness value
%BestSol------the optimal solution
%Developed in MATLAB R2022a by  Abdel-Basset et al.                                                        %
%_______________________________________________________________________________________%

function [BestFitness,BestSol,cgcurve]=EDO(N,Max_iter,LB,UB,Dim,F_obj)
% [h total] = hist2d(Imd);
% %
% Lmax=256;
% for i =1:Lmax
%     for j=1:Lmax
%         probR(i,j)=h(i,j)/total;
%     end
% end
%% =========================== %%
...
%              V(i,:)=(Xwinners(i,:)+(z3(i).*Z1+(1-c).*Z2)-M); %Eq.(20)
        end
        %check bounds of V
        F_UB=V(i,:)>UB;
        F_LB=V(i,:)<LB;
    ...
    if mod(iter,1666)==0
        disp(['Iteration ' num2str(iter) ': Best Cost = ' num2str(BestFitness,20)]);
    end
    iter=iter+1;
end
end